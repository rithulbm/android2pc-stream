...
bool refresh_receiver_status(obs_properties_t *, obs_property_t *, void *)
{
    return true;
}
...
public:
    ReceiverSource(obs_data_t *settings, obs_source_t *source) : source_(source)
    {
        ...
        const std::wstring pipe_name = L"\\\\.\\pipe\\local-camera-receiver-" +
                                       std::wstring(pipe_id.begin(), pipe_id.end());
        ...
        api.data_set_bool(child_settings, "hw_decode", true);
    ...
    ~ReceiverSource()
    {
        ...
        if (child_active_.exchange(false) && child_) {
    ...
    {
        listen_when_hidden_.store(obs_api().data_get_bool(settings, kListenHiddenSetting));
    }

    void render() noexcept
    {
        if (child_) obs_api().source_video_render(child_);
    }

    [[nodiscard]] std::uint32_t width() const noexcept
    {
        return child_ ? obs_api().source_get_width(child_) : 0;
    }

    [[nodiscard]] std::uint32_t height() const noexcept
    {
        return child_ ? obs_api().source_get_height(child_) : 0;
    }

    [[nodiscard]] bool audio_render(
        std::uint64_t *timestamp_out,
        obs_source_audio_mix *audio_output,
        std::uint32_t mixers,
        std::size_t channels,
        std::size_t sample_rate) const noexcept
    {
        ...
        for (std::size_t mix = 0; mix < MAX_AUDIO_MIXES; ++mix) {
            ...
            for (std::size_t channel = 0; channel < channels; ++channel) {
                std::memcpy(
                    audio_output->output[mix].data[channel],
                    child_audio.output[mix].data[channel],
...
private:
    void watch_config() noexcept
    {
        ...
        while (!stopping_.load()) {
            ...
            if (listener_started) {
                const ReceiverState state = listener_->status().state;
                if (state == ReceiverState::failed || state == ReceiverState::stopped) {
                    listener_->stop();
...
{
    return static_cast<ReceiverSource *>(data)->audio_render(
        timestamp_out, audio_output, mixers, channels, sample_rate);
}
...
obs_properties_t *source_properties(void *data)
{
    ...
    obs_property_t *info = api.properties_add_text(
        properties,
        "pairing_help",
        "No video yet? Show the pairing QR and scan it in the phone app. Streaming starts automatically.",
        OBS_TEXT_INFO);
    if (info != nullptr) {
        api.property_set_long_description(
            info,
